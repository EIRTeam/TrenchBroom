...
MapReader::MapReader(
  ...
  const Model::MapFormat targetMapFormat, const Model::EntityPropertyConfig& entityPropertyConfig)
...
void MapReader::onBeginEntity(
  const size_t /* line */, std::vector<Model::EntityProperty> properties,
  ParserStatus& /* status */) {
...
void MapReader::onStandardBrushFace(
  const size_t line, const Model::MapFormat targetMapFormat, const vm::vec3& point1,
  const vm::vec3& point2, const vm::vec3& point3, const Model::BrushFaceAttributes& attribs,
  ParserStatus& status) {
...
void MapReader::onValveBrushFace(
  const size_t line, const Model::MapFormat targetMapFormat, const vm::vec3& point1,
  const vm::vec3& point2, const vm::vec3& point3, const Model::BrushFaceAttributes& attribs,
  const vm::vec3& texAxisX, const vm::vec3& texAxisY, ParserStatus& status) {
...
void MapReader::onPatch(
  const size_t startLine, const size_t lineCount, Model::MapFormat, const size_t rowCount,
  const size_t columnCount, std::vector<vm::vec<FloatType, 5>> controlPoints,
  std::string textureName, ParserStatus&) {
  m_objectInfos.push_back(PatchInfo{
    rowCount, columnCount, std::move(controlPoints), std::move(textureName), startLine, lineCount,
    m_currentEntityInfo});
}
...
/**
 * Represents the parent information stored in ObjectInfo, which is either the index of the object
 * info for the parent, or container info read from entity properties.
 */
...
/**
 * Records issues that occured during node creation. These issues did not prevent node creation, but
 * they must be logged nevertheless.
 */
...
/**
 * Extracts container info (either a layer or a group ID) from the given entity properties if
 * present. In case of a malformed ID, an issue is added to the given vector of node issues and an
...
/**
 * Creates a world node for the given entity info and configures its default layer according to the
 * information in the entity attributes.
 */
static CreateNodeResult createWorldNode(
  MapReader::EntityInfo entityInfo, const Model::EntityPropertyConfig& entityPropertyConfig,
...
/**
 * Creates a layer node for the given entity info. Returns an error if the entity attributes contain
...
/**
 * Creates a group node for the given entity info. Returns an error if the entity attributes contain
 * missing or invalid information.
 */
...
/**
 * Creates a world, layer, group or entity node depending on the information stored in the given
 * entity info.
 *
...
/**
 * Creates a brush node from the given brush info. Returns an error if the brush could not be
 * created.
 */
static CreateNodeResult createBrushNode(
  ...
  return Model::Brush::create(worldBounds, std::move(brushInfo.faces), std::move(brushInfo.vertexColors))
    .and_then([&](Model::Brush&& brush) {
      ...
      auto parentInfo =
        brushInfo.parentIndex ? ParentInfo{*brushInfo.parentIndex} : std::optional<ParentInfo>{};
      return NodeInfo{
...
static CreateNodeResult createPatchNode(MapReader::PatchInfo patchInfo) {
  auto patchNode = std::make_unique<Model::PatchNode>(Model::BezierPatch{
    patchInfo.rowCount, patchInfo.columnCount, std::move(patchInfo.controlPoints),
    std::move(patchInfo.textureName)});
  patchNode->setFilePosition(patchInfo.startLine, patchInfo.lineCount);

  auto parentInfo =
    patchInfo.parentIndex ? ParentInfo{*patchInfo.parentIndex} : std::optional<ParentInfo>{};
...
/**
 * Transforms the given object infos into a vector of node infos. The returned vector is sparse,
 * that is, it contains empty optionals in place of nodes that we failed to create. We need the
 * indices to remain correct because we use them to refer to parent nodes later.
 */
static std::vector<std::optional<NodeInfo>> createNodesFromObjectInfos(
  const Model::EntityPropertyConfig& entityPropertyConfig,
  std::vector<MapReader::ObjectInfo> objectInfos, const vm::bbox3& worldBounds,
  const Model::MapFormat mapFormat, ParserStatus& status) {
  // create nodes in parallel, moving data out of objectInfos
  // we store optionals in the result vector to make the elements default constructible, which is a
  // requirement for parallel transform
  std::vector<CreateNodeResult> createNodeResults =
    kdl::vec_parallel_transform(std::move(objectInfos), [&](auto&& objectInfo) -> CreateNodeResult {
      return std::visit(
        kdl::overload(
        ...
        [&](Model::LayerNode* layerNode) {
          ...
          if (!layerIds.emplace(persistentId).second) {
            status.error(
              ...
              kdl::str_to_string("Skipping duplicate layer with ID '", persistentId, "'"));
            return true;
          }
          return false;
        },
        [&](Model::GroupNode* groupNode) {
          ...
          if (!groupIds.emplace(persistentId).second) {
            status.error(
              ...
              kdl::str_to_string("Skipping duplicate group with ID '", persistentId, "'"));
            return true;
          }
          return false;
        },
            ...
            kdl::overload(
              [&](const MalformedTransformationIssue& m) {
                status.warn(
                  ...
                  kdl::str_to_string(
                    "Not linking group: malformed transformation '", m.transformationStr, "'"));
              },
              [&](const InvalidContainerId& c) {
                const auto containerName = c.type == ContainerType::Layer ? "layer" : "group";
                status.warn(
                  nodeInfo->node->lineNumber(), kdl::str_to_string(
                                                  "Adding object to default layer: Invalid ",
                                                  containerName, " ID '", c.idStr, "'"));
              }),
            issue);
        }
...
/**
 * Builds a map of nodes to their intended parents using the parent info stored in each node info
 * object (this either refers to a parent node by index or a parent layer or group by ID).
 *
 * Not every node comes with parent information, so the returned map does not contain entries for
              ...
              if (containerInfo.type == ContainerType::Layer) {
                status.warn(
                  nodeInfo->node->lineNumber(), kdl::str_to_string(
                                                  ...
                                                  containerInfo.id, "', adding to default layer"));
              } else {
                status.warn(
                  nodeInfo->node->lineNumber(), kdl::str_to_string(
 ...
 *
 * Brushes should be added to the node corresponding to the preceding recorded entity info. We
 * stored the index of the preceding entity info for each brush, so we can determine the parent node
 * for a brush using that index.
 *
 * Group and entity nodes can belong to the default layer, a custom layer or another group. If such
 * a node belongs to a custom layer or a group, the ID of the containing layer or group is stored in
 * the entity properties of the entity info from which the node was created. Since the entity
 * properties of these nodes are discarded when the node is created, we record this information in a
 * separate map before creating we create nodes. We later use it to find the parent layer or group
 * of a group or entity node.
 *
 * Nodes for which the parent node is not known (e.g. when parsing only brushes) are added to a
 * default parent, which is returned from the `onWorldNode` callback.
 */
void MapReader::createNodes(ParserStatus& status) {
  ...
  auto nodeInfos = createNodesFromObjectInfos(
    m_entityPropertyConfig, std::move(m_objectInfos), m_worldBounds, m_targetMapFormat, status);

  // call onWorldNode for the first world node, remember the default parent and clear out all other
  // world nodes the brushes belonging to redundant world nodes will be added to the default parent
